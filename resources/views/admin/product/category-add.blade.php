...
@section('content')
  <div class="Huiform">
    <form action="{{ route('product-category-operate') }}" method="post" id="form">
      <table class="table table-bg">
        <tbody>
        <tr>
            <th width="100" class="text-r"><span class="c-red">*</span> 选择分类：</th>
            <td>
                <select class="select" name="pid" style="width: 200px;">
                    <option value="0">等级分类</option>
                    @foreach($category as $vo)
                        <option value="{{ $vo['id'] }}" {{ $info['pid']==$vo['id']?'selected':'' }}>{{ $vo['name'] }}</option>
                    @endforeach
                </select>
            </td>
        </tr>
...
@section('script')
  ...
  <script>
      ...
      var uploader = WebUploader.create({
          ...
          method: 'POST',
          // 文件接收服务端。
          server: '{{ route('upload-image',['type'=>'category']) }}',
          // 选择文件的按钮。可选。
